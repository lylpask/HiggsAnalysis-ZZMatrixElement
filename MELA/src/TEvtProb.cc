...
double TEvtProb::XsecCalc_XVV(
  ...
  // ME calculations
  if (forceUseMCFM) dXsec = SumMatrixElementPDF(_process, _production, _matrixElement, &event_scales, &RcdME, EBEAM, (selfDSpinZeroCoupl.Hvvcoupl_freenorm), verbosity);
    ...
    if (isSpinZero || isSpinOne || isSpinTwo) dXsec = JHUGenMatEl(_process, _production, &RcdME);
    else cerr
      << "TEvtProb::XsecCalc_XVV: JHUGen ME is not spin zero, one or two! The process is described by "
      << "Process: " << _process << ", Production: " << _production << ", and ME: " << _matrixElement
      << endl;
  } // end of JHUGen matrix element calculations

  if (verbosity >= TVar::DEBUG) cout
    << "Process " << TVar::ProcessName(_process)
...
double TEvtProb::XsecCalc_VVXVV(
  ...
  // ME calculation
  if (forceUseMCFM) dXsec = SumMatrixElementPDF(_process, _production, _matrixElement, &event_scales, &mcfm_event, &RcdME, EBEAM, (selfDSpinZeroCoupl.Hvvcoupl_freenorm), verbosity);
...
double TEvtProb::XsecCalcXJJ(TVar::Process proc, TVar::Production production, TVar::VerbosityLevel verbosity){
  ...
  if (_matrixElement == TVar::JHUGen){
    dXsec = HJJMatEl(_process, _production, _matrixElement, &event_scales, &RcdME, verbosity, EBEAM);
    if (verbosity >= TVar::DEBUG) cout <<"Process " << TVar::ProcessName(_process) << " TEvtProb::XsecCalcXJJ(): dXsec=" << dXsec << endl;
  }
...
double TEvtProb::XsecCalcXJ(TVar::Process proc, TVar::Production production, TVar::VerbosityLevel verbosity){
  ...
  if (_matrixElement == TVar::JHUGen){
    ...
    if (verbosity >= TVar::DEBUG) std::cout << "Process " << TVar::ProcessName(_process) << " TEvtProb::XsecCalcXJ(): dXsec=" << dXsec << endl;
  }
...
double TEvtProb::XsecCalc_VX(
  ...
  if (_matrixElement == TVar::JHUGen){
    ...
    dXsec = VHiggsMatEl(_process, _production, _matrixElement, &event_scales, &RcdME, EBEAM, verbosity);
    if (verbosity >= TVar::DEBUG) std::cout << "Process " << TVar::ProcessName(_process) << " TEvtProb::XsecCalc_VX(): dXsec=" << dXsec << endl;
  } // end of JHUGen matrix element calculations
...
double TEvtProb::XsecCalc_TTX(
  ...
  if (_matrixElement == TVar::JHUGen){
    ...
    if (_production==TVar::ttH) dXsec = TTHiggsMatEl(_process, _production, _matrixElement, &event_scales, &RcdME, EBEAM, topDecay, topProcess, verbosity);
    else if (_production==TVar::bbH) dXsec = BBHiggsMatEl(_process, _production, _matrixElement, &event_scales, &RcdME, EBEAM, topProcess, verbosity);
    if (verbosity >= TVar::DEBUG) std::cout << "Process " << TVar::ProcessName(_process) << " TEvtProb::XsecCalc_TTX(): dXsec=" << dXsec << endl;
  }
